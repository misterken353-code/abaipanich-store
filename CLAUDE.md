...
**เป้าหมาย:** user อยากได้ระบบรับสมัครคนขับส่งของหลายคนแบบ Lalamove/Grab — ตัดสินใจแล้วว่าทำแบบเบา (ไม่ใช่ marketplace เต็มรูปแบบที่คนขับสมัคร/กดรับงานเอง) เพราะร้านมีขนาดเล็ก แอดมินคนเดียวจัดการได้ทัน: มี "ทำเนียบคนขับ" ให้แอดมินเพิ่มเอง แล้ว**แอดมินเป็นคนเลือกมอบหมายงานให้คนขับแต่ละคนเอง** ไม่มีการดันงานอัตโนมัติ/จับคู่ตามระยะทาง/ระบบคะแนน/ค่าคอมมิชชั่นใดๆ
...
- หน้า `admin/orders/[id]` เพิ่มส่วน "มอบหมายคนขับ" — dropdown เลือกคนขับที่ `isActive` (โชว์ "— ไม่มี LINE" ถ้าคนขับยังไม่ผูก LINE), กด "มอบหมายงาน" → `POST /api/admin/orders/[id]/assign-rider` ตั้ง `riderId`+`assignedAt` แล้วถ้าคนขับมี `lineUserId` จะ push ข้อความงานให้ทันที: "🛵 งานส่งใหม่ ออเดอร์ {orderNo}" พร้อมชื่อ/เบอร์ลูกค้า, รายการสินค้า, วิธีจัดส่ง, ที่อยู่+ลิงก์ Google Maps (ถ้ามีพิกัด), ยอดเก็บเงินปลายทาง (ถ้า `paymentMethod === COD`)
...
- ทดสอบเต็ม flow บน dev server: เพิ่มคนขับทดสอบ (ไม่มี LINE) → toggle เปิด/ปิดใช้งานได้จริง → มอบหมายให้ออเดอร์ทดสอบ → เห็นข้อความ "มอบหมายแล้ว (คนขับไม่มี LINE ผูกไว้ จึงไม่ได้แจ้งเตือน)" ถูกต้อง → ลบ order/customer/rider ทดสอบออกจาก DB แล้ว (ไม่ได้แตะต้องออเดอร์จริงของร้าน)
- **ถ้าในอนาคตอยากขยายเป็น marketplace เต็มรูปแบบ** (คนขับสมัคร/login เอง, กดรับงานเอง, จับคู่ตามระยะทาง, ระบบค่าคอมมิชชั่น/payout, rating) จะเป็นงานใหญ่กว่านี้มาก ต้องคุยเรื่อง scope ใหม่ทั้งหมด
