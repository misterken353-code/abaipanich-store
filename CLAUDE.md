...
### ✅ คำนวณค่าส่งม้าเร็วอัตโนมัติตามระยะทางจริง + ติดตามเงินสด COD ที่คนขับถืออยู่ (เสร็จ, 2026-07-09)
**เป้าหมาย:** user อยากให้ระบบคนขับ "ใกล้เคียง Grab" มากขึ้นเฉพาะจุดที่ยังขาดจริง — ค่าส่งเดิมเป็นแค่ข้อความสูตร ไม่เคยคำนวณจริง, ไม่มีการติดตามเงินสด COD ที่คนขับถืออยู่เลย (ระบบเดิมมีแค่ทิศทาง "ร้าน→คนขับ" ค่าคอมมิชชั่น ไม่มีทิศทาง "คนขับ→ร้าน" เงินค่าสินค้าที่เก็บจากลูกค้า) — user ยืนยัน (ผ่าน AskUserQuestion): ค่าส่งคำนวณอัตโนมัติแต่ **ยังจ่ายแยกให้คนขับเหมือนเดิม** (ไม่รวม totalAmount/QR), เงินสด COD **อยากให้ติดตาม+มีปุ่มยืนยันนำส่ง**

- Schema เพิ่ม: `AppSettings.storeLat`/`storeLng` (พิกัดร้าน), `Order.deliveryFee` (Decimal? คำนวณตอนสร้างออเดอร์), `Order.codRemitted`/`codRemittedAt` (คนขับนำเงินสด COD มาคืนร้านแล้วหรือยัง)
- `src/lib/deliveryFee.ts` — `calculateDeliveryFee(distanceKm)` (สูตรเดิม: 15 บาทกม.แรก, กม.ที่ 2 ขึ้นไปกม.ละ 5 บาท, ปัดขึ้นเป็นกม.) + `estimateDeliveryFee(...)` ใช้ `haversineKm` เดิมจาก `src/lib/geo.ts`
- `/admin/settings` เพิ่มช่องพิกัดร้าน + ปุ่ม "📍 ใช้ตำแหน่งปัจจุบัน" (geolocation แบบเดียวกับ checkout) — ถ้ายังไม่ตั้งพิกัด ระบบ fallback ไปโชว์สูตรข้อความเดิม ไม่ block การสั่งซื้อ
- `GET /api/delivery-fee/estimate?lat=&lng=` (public) — checkout เรียกแบบเรียลไทม์หลังลูกค้าแชร์โลเคชั่นสำหรับม้าเร็ว โชว์ "ค่าส่งโดยประมาณ ฿X (ระยะทาง ~Y กม.)"
- `POST /api/orders` คำนวณ `deliveryFee` ฝั่ง server เท่านั้น (ไม่เชื่อ client) ตอนสร้างออเดอร์จริง — แสดงผลที่คำนวณแล้วใน `/order/[orderNo]`, `/admin/orders/[id]`, และการ์ดงานใน `RiderDashboard.tsx` (แยกชัดเจนจาก COD: "💵 เก็บเงินค่าสินค้า...(นำส่งร้าน)" vs "💰 ค่าส่งเก็บจากลูกค้า...(ของคุณ)")
- **เงินสด COD:** `POST /api/admin/riders/[id]/settle-cod` (mirror `settle-commission` แต่กลับทิศทาง — `updateMany` ตั้ง `codRemitted=true` ทุกออเดอร์ COD/SHIPPED ที่ยังไม่นำส่งของคนขับคนนั้น) — คอลัมน์ "เงินสดค้างนำส่ง" + ปุ่ม "นำส่งเงินแล้ว" ใน `/admin/riders` (`RidersManager.tsx`) — **การนำส่งจริงต้อง admin กดยืนยันเท่านั้น ไม่มีปุ่มให้คนขับกดเอง** (กันคนขับกดหลอกว่านำส่งแล้ว) — rider dashboard แสดง "เงินสดค้างนำส่งร้าน" แบบ read-only เท่านั้น
- **แก้บั๊กที่เจอไปในตัว:** `unsettledCommission` ใน `GET /api/rider/[token]/orders` เดิมคำนวณจาก `history` ที่ `take: 20` จำกัดไว้ — ถ้าคนขับมีงานค้าง settle เกิน 20 งานยอดจะผิด (ฝั่ง admin ไม่มีปัญหานี้เพราะ query ไม่จำกัด) — แก้เป็น query แยกไม่จำกัดสำหรับทั้ง `unsettledCommission` และ `unsettledCod` ใหม่
- ทดสอบเต็ม flow บน dev server: ตั้งพิกัดร้าน → mock geolocation ลูกค้าห่าง ~9.5 กม. → checkout โชว์ "ค่าส่งโดยประมาณ ฿60" ถูกต้อง → สร้างออเดอร์ COD+MOTORCYCLE จริง → admin เห็นค่าส่ง 60 บาท → คนขับกดรับ+จัดส่งสำเร็จ → เห็น `unsettledCod: 4` ถูกต้อง (แยกจากค่าส่ง 60 ที่เป็นของคนขับเอง) → กด "นำส่งเงินแล้ว" → ยอดกลับเป็น 0 → ลบ order/customer/rider/admin ทดสอบออกจาก DB หมดแล้ว, เคลียร์พิกัดร้านทดสอบกลับเป็น null ด้วย (ไม่ใช่พิกัดจริง แค่ประมาณจังหวัดไว้ทดสอบสูตร) — **เจ้าของร้านต้องไปกดปุ่ม "ใช้ตำแหน่งปัจจุบัน" เองตอนอยู่ที่ร้านจริง ก่อนฟีเจอร์คำนวณค่าส่งจะทำงาน**
- **ระหว่างทดสอบเกิดอุบัติเหตุ:** เผลอกดปุ่ม "รับงานนี้" ด้วย DOM selector ที่กว้างเกินไป ทำให้ไปเคลม order จริงของลูกค้า (SP202607050003, "ตุ้ย") ด้วยคนขับทดสอบโดยไม่ตั้งใจ — แก้ทันทีด้วยการ set `riderId`/`assignedAt` กลับเป็น `null` ตรงๆ ผ่าน DB (ยืนยันค่าอื่นไม่เปลี่ยน) แต่ order นี้ได้รับ LINE push แจ้งร้านว่า "คนขับทดสอบ" รับงานไปแล้ว 1 ข้อความที่เป็น false positive — **บทเรียน: ตอนทดสอบ claim/deliver ผ่าน rider dashboard ที่มีออเดอร์จริงปนอยู่ ให้เรียก API ตรงด้วย orderId ที่รู้แน่ชัด (`fetch('/api/rider/{token}/claim', {body: {orderId}})`) แทนการคลิกปุ่มผ่าน DOM selector เสมอ**

### ✅ ระบบสมัครม้าเร็ว — คนทั่วไปสมัครเป็นคนขับเองผ่านเว็บ (เสร็จ, 2026-07-10)
**เป้าหมาย:** user อยากให้คนทั่วไปสมัครเป็นม้าเร็วได้เอง — คีย์ข้อมูล + แนบรูปบัตรประชาชน + เบอร์โทร แล้วเพิ่มเพื่อน LINE OA มาคุยงานต่อทาง LINE เลย

- Schema เพิ่ม: model `RiderApplication { name, phone, idCardImage (Text, data URL), note?, status PENDING/APPROVED/REJECTED, riderId?, reviewedAt }` + `AppSettings.lineOaUrl` (ลิงก์เพิ่มเพื่อน LINE OA เช่น https://lin.ee/xxx)
- `/apply-rider` (public) — `page.tsx` + `ApplyRiderForm.tsx`: ฟอร์มชื่อ/เบอร์/รูปบัตรประชาชน/หมายเหตุ — **รูปบีบอัดฝั่ง client ด้วย canvas** (ย่อไม่เกิน 1280px, JPEG q0.8) เพราะยังไม่มี Blob storage เก็บเป็น data URL ใน DB แทน (จำกัด ≤2MB ฝั่ง server) — สมัครสำเร็จโชว์ปุ่มเขียว LINE "เพิ่มเพื่อน" (จาก `lineOaUrl`; ถ้ายังไม่ตั้งจะ fallback เป็นข้อความบอกให้ค้นหา OA เอง)
- `POST /api/rider-applications` (public) — validate เบอร์ 9-10 หลัก, กันสมัครซ้ำ (มี PENDING ค้างอยู่ = 409, เบอร์เป็น Rider อยู่แล้ว = 409), แจ้งร้านผ่าน `notifyShop()` ทันทีที่มีผู้สมัครใหม่
- `/admin/riders` เพิ่ม `ApplicationsPanel.tsx` (แถบเหลืองด้านบน แสดงเฉพาะเมื่อมีใบสมัคร PENDING) — ดูรูปบัตรเต็มจอด้วย `ImageLightbox` เดิม, ปุ่ม "อนุมัติเป็นคนขับ" / "ปฏิเสธ" / "ลบ" + โชว์ลิงก์หน้าสมัครให้แอดมินคัดลอกไปแชร์
- `PATCH /api/admin/rider-applications/[id]` — approve สร้าง `Rider` จากชื่อ+เบอร์ในทรานแซกชันเดียว (ได้ accessToken อัตโนมัติ — แอดมินค่อยคัดลอกลิงก์รับงานส่งให้ทาง LINE), reject แค่เปลี่ยนสถานะ; `DELETE` ลบใบสมัครทิ้ง (ลบรูปบัตร = ข้อมูลอ่อนไหว ควรลบหลังตรวจเสร็จ)
- `/admin/settings` เพิ่มช่อง "ลิงก์เพิ่มเพื่อน LINE OA" (ดูจาก LINE OA Manager > เพิ่มเพื่อน > คัดลอก URL) — **ยังไม่ได้ตั้งค่าจริง user ต้องไปกรอกเอง** ไม่งั้นหน้า success จะไม่มีปุ่มเพิ่มเพื่อน
- Footer หน้าแรก (`StorefrontClient.tsx`) เพิ่มลิงก์ "🏍️ รับสมัครม้าเร็ว — สมัครที่นี่"
- ทดสอบเต็ม flow บน dev server แล้ว: สมัครผ่านฟอร์มจริง (แนบไฟล์ → บีบอัดเป็น JPEG → success screen), กันสมัครซ้ำ 409, อนุมัติในหน้าแอดมิน → Rider ถูกสร้าง + ใบสมัครหายจากคิว, settings round-trip ไม่ทับ field อื่น — ลบข้อมูลทดสอบทั้งหมดออกแล้ว (ใบสมัคร 2, rider 1, test admin 1, LINE log 2) — **หมายเหตุ: ระหว่างทดสอบมี LINE push แจ้ง "ผู้สมัครใหม่" หลอก 2 ข้อความเข้า LINE เจ้าของร้าน**
