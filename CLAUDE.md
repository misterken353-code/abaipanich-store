...
- `checkout/page.tsx` เรียง field ใหม่ให้ **เบอร์โทรศัพท์อยู่บนสุด** — พอ blur ช่องเบอร์ (`onBlur`) จะเรียก lookup API ทันที ถ้าเจอลูกค้าเดิม: prefill ชื่อ/ที่อยู่/lineUserId ให้ + โชว์ banner "✓ พบข้อมูลลูกค้าเดิม" + **ล็อกช่องชื่อ/ที่อยู่เป็นสีเทา** (reuse pattern เดียวกับ `/admin/settings` — ต้องกด "แก้ไขข้อมูล" ก่อนถึงจะพิมพ์ทับได้)
